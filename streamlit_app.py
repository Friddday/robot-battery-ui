...
.phone {
    ...
    box-shadow:
        0 30px 80px rgba(50, 33, 18, 0.28),
        0 8px 20px rgba(50, 33, 18, 0.16);
}
...
.room.charging .charge-ring {
    ...
    animation:
        ringSpin 1.05s linear infinite;
}

.room.charging .robot {
    animation:
        robotCharge 0.85s ease-in-out infinite;
}

.room.low .robot {
    animation:
        robotLow 0.48s linear infinite;
}

.room.celebrate .robot {
    animation:
...
function render() {
    ...
    $("cleanTime").textContent =
        cleanMinutes();

    $("pointer").style.left =
        state.soc + "%";

    $("cleanFill").style.width =
        state.progress + "%";
...
function takePhoto() {
    ...
    openModal(
        "오늘의 사진",
        "왕관을 쓴 몽글이의 사진을 촬영했습니다." +
        "<br><br>" +
        "향후 장식 아이템과 청소 완료 장면을 " +
        "사진첩에 저장할 수 있습니다."
    );
}
